          ...
          {data && view === "overview" && <>
            ...
            <section className="commerce-primary-grid">
              <article className="commerce-panel revenue-panel"><header><div><h2>Receita por dia</h2><p>Desempenho consolidado dos canais</p></div><span>Últimos 7 dias</span></header>
                {chart ? <div className="sales-chart"><div className="chart-scale"><span>{currency(chart.max)}</span><span>{currency(chart.max / 2)}</span><span>R$ 0</span></div><div className="chart-canvas"><svg viewBox="0 0 100 100" preserveAspectRatio="none"><defs><linearGradient id="salesFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#F06A2C" stopOpacity=".3"/><stop offset="1" stopColor="#F06A2C" stopOpacity="0"/></linearGradient></defs><polygon points={`0,100 ${chart.points} 100,100`} fill="url(#salesFill)"/><polyline points={chart.points} fill="none" stroke="#F06A2C" strokeWidth="2.2" vectorEffect="non-scaling-stroke"/></svg><div className="chart-days">{data.trend.map((point) => <span key={point.day}>{formatStoredDate(point.day, weekday).replace(".", "")}</span>)}</div></div></div> : <EmptyChart />}
              </article>
